...
2. **No inspectable contract.** There was no single file that answered "which
   events does the visualizer capture?" You had to reverse-engineer it from
   individual scripts or source code.

We fixed both by introducing a **dedicated manifest** —
`.github/hooks/visualizer-hooks.json` — that bootstrap always creates. It
declares every covered event type with its corresponding hook command:

```json
{
  "version": 1,
  "description": "Auto-generated by Copilot Agent Activity Visualizer bootstrap.",
  "hooks": {
    "sessionStart": [{ "type": "command", "bash": "./.github/hooks/session-start.sh", "cwd": ".", "timeoutSec": 15 }],
    "preToolUse":   [{ "type": "command", "bash": "./.github/hooks/pre-tool-use.sh",  "cwd": ".", "timeoutSec": 10 }],
    "errorOccurred":[{ "type": "command", "bash": "./.github/hooks/error-occurred.sh", "cwd": ".", "timeoutSec": 10 }]
  }
}
```
...
- **Single source of truth.** Anyone — human or tool — can read one file to
  understand the visualizer's surface area.
...
- **Unbootstrap-safe.** Removing integration is as clean as deleting the
  manifest. User-managed manifests (like `ejs-hooks.json`) are handled
...
visualizer captures — no implicit wiring, no silent gaps.
...
| 9 | Keep hooks lightweight — capture the moment, process it elsewhere |
